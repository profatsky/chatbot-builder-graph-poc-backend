...
class InputRepository:
    ...
    async def get_input_by_id(self, group_id: UUID, input_id: UUID) -> Optional[InputReadSchema]:
        input_field = await self._session.execute(
            select(InputModel)
            .where(
                InputModel.group_id == group_id,
                InputModel.input_id == input_id,
            )
        )
    ...
    async def delete_input(self, group_id: UUID, input_id: UUID):
